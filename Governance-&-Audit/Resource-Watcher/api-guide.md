...
| Key         | Type   | Required | Description                         |
| ----------- | ------ | -------- | ----------------------------------- |
| alarmName   | String | Yes      | <p>알림 이름<br>최대 255 글자까지 등록 가능</p>   |
| description | String | No       | <p>알림 설명<br>최대 1,000 글자까지 등록 가능</p> |

* AlarmTarget

| Key                 | Type   | Required | Description                                                                                                                                                                                                                                |
| ------------------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| alarmTargetTypeCode | String | Yes      | <p>알림 대상 타입<br>1. <code>UUID</code>: 단일 조직 멤버 대상(NHN cloud 회원, IAM 멤버)<br>2. <code>ROLE</code>: 역할 대상(조직 역할, 프로젝트 역할 그룹, 프로젝트 역할)<br>3. <code>ALARM_KEY</code>: 알림 수신 그룹 대상(조직 알림 수신 그룹, 프로젝트 알림 수신 그룹)<br>4. <code>WEBHOOK</code>: 웹훅</p> |
| alarmTarget         | String | No       | <p>알림 대상 정보<br>&#x3C;타입별 설정 값><br>1. UUID: <code>멤버 UUID</code><br>2. ROLE: <code>역할 ID</code>(ex. ADMIN)<br>3. ALARM_KEY: <code>알림 수신 그룹 ID</code><br>4. WEBHOOK: <code>입력하지 않음</code></p>                                                |
| emailAlarm          | String | No       | <p>이메일 수신 여부<br>1. <code>Y</code>: 이메일 수신<br>2. <code>N</code>: 이메일 수신하지 않음<br>WEBHOOK, ALARM_KEY는 입력하지 않음.</p>                                                                                                                            |
| smsAlarm            | String | No       | <p>SMS 수신 여부<br>1. <code>Y</code>: SMS 수신<br>2. <code>N</code>: SMS 수신하지 않음<br>WEBHOOK, ALARM_KEY는 입력하지 않음.</p>                                                                                                                            |
| webhookUrl          | String | No       | <p>Webhook URL 주소<br>http:// 또는 https://로 시작해야 함<br>알림 대상 타입 <code>WEBHOOK</code> 설정 시 입력</p>                                                                                                                                              |
| webhookSecret       | String | No       | <p>Webhook 비밀 키<br>알림 대상 타입 <code>WEBHOOK</code> 설정 시 입력</p>                                                                                                                                                                               |
...
productId, eventId 값은 [3.1 이벤트 목록 조회 API 응답값](api-guide.md#list-event-response-event) 을 참고하십시오.
